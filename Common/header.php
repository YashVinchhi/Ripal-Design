...
<style>
  ...
  .menu-btn .menu-line { background: var(--header-text-color); }
    /* Place hamburger neatly next to logo when grouped */
    .menu-btn { align-items: center; margin-right: 8px; }

  /* Overlay nav should remain dark and use white text for legibility */
  #navOverlay { background: var(--overlay-bg); }
  #navOverlay a { color: #fff !important; }

  /* Ensure logo image remains visible regardless of header background */
  .mirrored-logo img { filter: none; }
</style>
...
<script>
  ...
  function open(){ sidebar.style.right = '0'; backdrop.style.opacity = '1'; backdrop.style.pointerEvents='auto'; menu.classList.add('is-active'); }
  function close(){ sidebar.style.right = '-320px'; backdrop.style.opacity = '0'; backdrop.style.pointerEvents='none'; menu.classList.remove('is-active'); }
  if(menu){ menu.addEventListener('click', function(){ if(sidebar.style.right === '0px') close(); else open(); }); }
