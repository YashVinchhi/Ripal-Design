...
<style>
  :root { --alt-overlay-bg: rgba(115,18,9,0.96); --alt-text: #fff; }
  nav.alt-header { position:fixed; top:0; left:0; right:0; height:64px; display:flex; align-items:center; padding:0 20px; z-index:9999; background:transparent; }
  .alt-logo { display:flex; align-items:center; gap:12px; }
  .alt-menu { margin-left:auto; }
  .alt-menu .alt-btn { background:transparent; border:0; color:var(--alt-text); font-size:18px; padding:8px; }
  /* Overlay becomes a right-side sliding panel that occupies max 25% of screen */
  #altOverlay { position:fixed; inset:0; background:rgba(0,0,0,0.25); opacity:0; pointer-events:none; transition:opacity .22s ease; z-index:10000; }
  #altOverlay.open { opacity:1; pointer-events:auto; }
  .alt-panel { position:absolute; right:0; top:0; height:100%; width:min(25%,360px); max-width:360px; background:var(--alt-overlay-bg); box-shadow:-8px 0 24px rgba(0,0,0,0.2); transform:translateX(100%); transition:transform .28s ease; display:flex; flex-direction:column; overflow:hidden; }
  #altOverlay.open .alt-panel { transform:translateX(0); }
  .alt-panel { --spot-x: 50%; --spot-y: 50%; }
  .alt-panel::before{
    /* subtle grid pattern */
    content: '';
    position: absolute;
  ...
  .alt-panel nav { position:relative; z-index:2; display:flex; flex-direction:column; gap:14px; padding:32px; padding-top:48px; overflow:auto; }
  .alt-panel a { color:var(--alt-text); font-size:18px; text-decoration:none; }
  .alt-panel .panel-footer { margin-top:auto; padding:20px; display:flex; gap:10px; justify-content:center; z-index:2; }
  .btn-alt { padding:10px 14px; border-radius:8px; font-weight:600; border:2px solid rgba(255,255,255,0.12); }
  .btn-login { background:transparent; color:var(--alt-text); }
  .btn-signup { background:#fff; color:var(--alt-overlay-bg); }
  /* Ensure sign-up button text overrides the general anchor color inside panel */
  .alt-panel .panel-footer .btn-signup { color: var(--alt-overlay-bg); }
  .alt-hamburger { width:36px; height:24px; display:inline-flex; flex-direction:column; justify-content:space-between; }
  .alt-hamburger span { display:block; height:3px; background:var(--alt-text); border-radius:2px; }
</style>
...
<script>
  (function(){
    ...
    if(panel){
      ...
      panel.addEventListener('mousemove', function(e){ mx = e.clientX; my = e.clientY; if(!rafId) rafId = requestAnimationFrame(update); });
      panel.addEventListener('touchmove', function(e){ if(e.touches && e.touches[0]){ mx = e.touches[0].clientX; my = e.touches[0].clientY; if(!rafId) rafId = requestAnimationFrame(update); } }, {passive:true});
      panel.addEventListener('mouseleave', function(){ rect = null; panel.style.setProperty('--spot-x', '50%'); panel.style.setProperty('--spot-y', '50%'); });
    }
  })();
</script>
