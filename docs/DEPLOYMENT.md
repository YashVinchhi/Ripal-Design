...
- `.github/workflows/ci.yml` — CI: installs deps, runs `php -l`, builds and pushes Docker image to GHCR.
...
How it works
1. Push to `Prod` (or `main` / `ripal-design`) — `ci.yml` runs: composer install, php lint, builds Docker image and pushes to GHCR with `:latest` and `:${{ github.sha }}` tags.
