...
if (isset($_POST['submit'])) {
    ...
    try {
        ...
        $_SESSION['contact_form_error'] = 'Failed to send message. Please try again.';
    }
            ...
            <form class="max-w-lg w-full mx-0 space-y-8" action="<?php echo htmlspecialchars($contactPageUrl, ENT_QUOTES, 'UTF-8'); ?>" method="POST" id="contactForm" >
                ...
                <div class="group">
                    ...
                    <select name="subject" class="w-full py-3 text-lg border-b border-white/20 bg-transparent focus:border-[#731209] outline-none transition-colors text-gray-300" data-validation="required select">
                        <option value="" class="bg-black" <?php echo (($old_input['subject'] ?? '') === '') ? 'selected' : ''; ?>>Select Inquiry Type</option>
                        <option value="residential" class="bg-black" <?php echo (($old_input['subject'] ?? '') === 'residential') ? 'selected' : ''; ?>>Residential Project</option>
                        <option value="commercial" class="bg-black" <?php echo (($old_input['subject'] ?? '') === 'commercial') ? 'selected' : ''; ?>>Commercial Project</option>
                        <option value="consultation" class="bg-black" <?php echo (($old_input['subject'] ?? '') === 'consultation') ? 'selected' : ''; ?>>Design Consultation</option>
                        <option value="other" class="bg-black" <?php echo (($old_input['subject'] ?? '') === 'other') ? 'selected' : ''; ?>>Other</option>
                    </select>
